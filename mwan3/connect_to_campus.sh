...
for i in $(seq 1 4)
do
	routerIP=$(python3 getVlanIP.py macvlan$i)
	Cookies=$(python3 getSessionId.py $routerIP)
	JSESSIONID=""
	route=""
	eval $(echo $Cookies | awk '{printf("JSESSIONID=%s;route=%s",$1,$2)}')
	UserInfo=$(python3 getUserInfo.py userinfo$i.json)
	UserName=""
	Password=""
	SchoolId=""
	BaseIP=""
	eval $(echo $UserInfo | awk '{printf("UserName=%s;Password=%s;SchoolId=%s;BaseIP=%s",$1,$2,$3,$4)}')
	curl --interface macvlan$i $'http://139.198.3.98/sdjd/protalAction\u0021portalAuth.action' \
